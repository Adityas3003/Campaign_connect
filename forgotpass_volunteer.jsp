    ...
    if (email != null && newPassword != null && confirmPassword != null) {
        if (!newPassword.equals(confirmPassword)) {
            ...
            try {
                ...
                PreparedStatement checkStmt = con.prepareStatement("SELECT * FROM volunteers WHERE email=?");
                checkStmt.setString(1, email);
                ResultSet rs = checkStmt.executeQuery();

                if (rs.next()) {
                    // Update password
                    PreparedStatement updateStmt = con.prepareStatement("UPDATE volunteers SET password=? WHERE email=?");
